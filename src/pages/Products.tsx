...
function CategorySection({ cat, whatsapp, products_page }: { cat: ReturnType<typeof useContent>['product_categories'][0]; whatsapp: string; products_page: ReturnType<typeof useContent>['products_page'] }) {
  return (
    <div className="cat-section">
      ...
      <div className="cat-cta-row">
        <a href={`${whatsapp}?text=I'm interested in ${encodeURIComponent(cat.title)}`} target="_blank" rel="noopener noreferrer" className="cat-cta-wa">
          <MessageCircle className="w-4 h-4" /> {products_page.btn_whatsapp}
        </a>
        <Link to="/contact" className="cat-cta-quote">{products_page.btn_quote} <ArrowRight className="w-4 h-4" /></Link>
      </div>
    </div>
  );
}
...
export function Products() {
  ...
  const { product_categories, site, products_page } = content;
  ...
  return (
    <div className="products-page">
      ...
      <section className="products-content section-reveal">
        {activeCat && <CategorySection key={activeCat.id} cat={activeCat} whatsapp={site.whatsapp_number} products_page={products_page} />}
      </section>

      <section className="all-cats-strip section-reveal">
        <div className="all-cats-inner">
          <div className="all-cats-title">{products_page.strip_title}</div>
